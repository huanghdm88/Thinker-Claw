...
| `getState()` | — | 返回当前 `{ characterIndex, character, actionIndex, actionId, evolved, language, activityWidth, activityHeight }` |
| `getCharacters()` | — | 返回角色数组副本 |
| `getActions()` | — | 返回动作列表 `{ id, name, index }[]`（name 随当前语言） |
| `setActivityArea(width, height)` | 数字（逻辑像素） | 设置角色活动区域（画布）宽高；**不得小于最小活动区域**，小于时自动钳位 |
| `getMinActivityArea()` | — | 返回 `{ width, height }`，即允许的最小活动区域（当前为 192×85） |

### 8.2 活动区域说明

- **活动区域**：即角色与道具的绘制区域（画布）宽高，随屏幕或容器变化时，角色与道具会**等比例缩放**。
- **最小活动区域**：为保证角色、动作与道具完整显示，活动区域不得小于 `getMinActivityArea()` 返回值（当前为 **192×85** 逻辑像素）。其他项目调用 `setActivityArea(width, height)` 时，若传入值小于最小值会自动钳位到最小值。
- **随角色尺寸变化**：活动区域变大时，角色与道具同步等比例放大；变小时同步缩小。
- **嵌入时**：若不调用 `setActivityArea`，活动区域由容器（如 iframe）尺寸自动计算（仍不低于最小活动区域）；若调用 `setActivityArea(w, h)`，则使用指定尺寸（≥ 最小），且不再随容器 resize 变化。

### 8.3 使用示例（同页或同域 iframe 内）
...
// state.characterIndex, state.actionId, state.evolved, state.language, state.activityWidth, state.activityHeight 等

// 设置活动区域（其他项目嵌入时可调，不能小于最小区域）
var min = window.ThinkerPet.getMinActivityArea();  // { width: 192, height: 85 }
window.ThinkerPet.setActivityArea(320, 142);      // 不小于 min 即可
...
{
  "type": "thinker-pet",
  "method": "setCharacter" | "setAction" | "setEvolved" | "setLanguage" | "applyState" | "setActivityArea" | "getMinActivityArea",
  "characterIndex": 0,
  "action": "dialogue",
  "evolved": true,
  "language": "en",
...
| setActivityArea | `width`, `height`（数字，逻辑像素；不能小于最小活动区域） | 设置角色活动区域，小于最小值时自动钳位 |
| getMinActivityArea | 无参数 | 子页面会回复 `postMessage`，`type: 'thinker-pet-min-area'`，带 `width`、`height` |
...
<script>
  ...
  // 设置活动区域（不小于 192×85）
  send({ method: 'setActivityArea', width: 320, height: 142 });
  // 获取最小活动区域（需监听 message 收 type: 'thinker-pet-min-area'）
  send({ method: 'getMinActivityArea' });
</script>
```
...
| 活动区域   | —            | `setActivityArea(w,h)` / `getMinActivityArea()` | `method: 'setActivityArea', width, height` / `getMinActivityArea`（回复 thinker-pet-min-area） |
